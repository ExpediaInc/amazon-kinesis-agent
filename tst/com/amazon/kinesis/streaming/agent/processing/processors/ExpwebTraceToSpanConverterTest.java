...
public class ExpwebTraceToSpanConverterTest {
    ...
    public void testServerTransform() throws Exception {
        final String input = loadTraceRecord("expweb-server-trace.txt");
        final Span output = Span.parseFrom(new ExpwebTraceToSpanConverter().convert(ByteBuffer.wrap(input.getBytes())));
        checkTransform(output, "3aca68e8-f128-461b-9adf-b5faaa6b0aac",
    ...
    public void testClientTransform() throws Exception {
        final String input = loadTraceRecord("expweb-client-trace.txt");
        final Span output = Span.parseFrom(new ExpwebTraceToSpanConverter().convert(ByteBuffer.wrap(input.getBytes())));
        checkTransform(output,"14761585-7e89-4a5e-b5db-117f69bbafaf",
    ...
    public void testTransformOfValuesWithQuote() throws Exception {
        final String input = loadTraceRecord("expweb-trace-quote.txt");
        final Span output = Span.parseFrom(new ExpwebTraceToSpanConverter().convert(ByteBuffer.wrap(input.getBytes())));
        checkTransform(output, "f8fc580e-701d-4b06-b1b7-df61ed7d9af4", "26cfcee0-ee13-4099-8066-b0f1571d8e33", "",
